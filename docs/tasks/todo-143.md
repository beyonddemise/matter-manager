...
- **`<svg><script>` never executed, in either run.** Assigned through `innerHTML`, a script
  ...
  wrong things.
- **`<use href="https://…">` was blocked by the same-origin rule, and reported no CSP
  violation at all.** It fails with `origin`, silently as far as any policy report is
  concerned. Anyone planning to monitor this through CSP reports would never see it attempted.
...
of which is not written in the policy and falls back correctly to `default-src 'none'`.
...
run, it exits 1 and names the vectors:
...
A hostile SVG got through the deployed policy:
  ...
  - inline in shadow root: executed foreignObject onerror=
  - inline in shadow root: executed javascript: href
```
