 ...
 * `test/ui/icons.browser.test.ts`, which resolves every icon the application uses and
...
const ALLOWED_IN_JS = new Map([
  ...
  [
    ...
    "Font Awesome's CDN in Web Awesome's default icon library, which " +
      'src/ui/icons/library.ts replaces before anything renders. Proved by ' +
...
if (failures.length > 0) {
  ...
  console.error(
    '\nThis application is offline-first (ADR 0002), so an asset behind a network request is ' +
      ...
      'Bundle it - see src/ui/fonts and src/ui/icons - or, if the string ' +
