...
const policy = (() => {
  ...
  if (line === undefined) throw new Error('No Content-Security-Policy in public/_headers')
...
if (external.size > 0) {
  ...
  console.error(
    '\nThis application is offline-first (ADR 0002): anything fetched at run time is ' +
      'missing exactly when it is needed most. Bundle it - see src/ui/fonts and ' +
      'src/ui/icons, and scripts/fetch-offline-assets.mjs.',
  )
  process.exit(1)
}

if (violations.length > 0) {
  console.error('The Content-Security-Policy in public/_headers blocked the application:\n')
